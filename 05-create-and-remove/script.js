...
const itemSpan = document.getElementById("item-count");
...
function updateCount() {
  itemSpan.textContent = itemUl.children.length;
}
...
addItemBtn.addEventListener("click", () => {
  const trimName = itemInput.value.trim();
  if (trimName === "") return;
  const li = document.createElement("li");
  li.textContent = trimName;
